const SAVE_USER = 'SAVE_USER';
const SAVE_USER_INPROGRESS = 'SAVE_USER_INPROGRESS';
const SAVE_USER_SUCCESS = 'SAVE_USER_SUCCESS';
const SAVE_USER_FAILURE = 'SAVE_USER_FAILURE';

const DELETE_USER = 'DELETE_USER';
const DELETE_USER_INPROGRESS = 'DELETE_USER_INPROGRESS';
const DELETE_USER_SUCCESS = 'DELETE_USER_SUCCESS';
const DELETE_USER_FAILURE = 'DELETE_USER_FAILURE';

const SAVE_ROLE = 'SAVE_ROLE';
const SAVE_ROLE_INPROGRESS = 'SAVE_ROLE_INPROGRESS';
const SAVE_ROLE_SUCCESS = 'SAVE_ROLE_SUCCESS';
const SAVE_ROLE_FAILURE = 'SAVE_ROLE_FAILURE';

const DELETE_ROLE = 'DELETE_ROLE';
const DELETE_ROLE_INPROGRESS = 'DELETE_ROLE_INPROGRESS';
const DELETE_ROLE_SUCCESS = 'DELETE_ROLE_SUCCESS';
const DELETE_ROLE_FAILURE = 'DELETE_ROLE_FAILURE';

const FETCH_USERS = 'FETCH_USERS';
const FETCH_USERS_FAILED = 'FETCH_USERS_FAILED';
const FETCH_USERS_SUCCESS = 'FETCH_USERS_SUCCESS';
const FETCH_USERS_PROGRESS = 'FETCH_USERS_PROGRESS';

const FETCH_ROLES = 'FETCH_ROLES';
const FETCH_ROLES_FAILED = 'FETCH_ROLES_FAILED';
const FETCH_ROLES_SUCCESS = 'FETCH_ROLES_SUCCESS';
const FETCH_ROLES_PROGRESS = 'FETCH_ROLES_PROGRESS';

const FETCH_STATUS = 'FETCH_STATUS';
const FETCH_STATUS_FAILED = 'FETCH_STATUS_FAILED';
const FETCH_STATUS_SUCCESS = 'FETCH_STATUS_SUCCESS';

const EXPAND_ACCOUNT_PANEL = 'EXPAND_ACCOUNT_PANEL';

export const actionTypes = {
  SAVE_ROLE_FAILURE,
  SAVE_ROLE_SUCCESS,
  SAVE_ROLE_INPROGRESS,
  SAVE_ROLE,
  DELETE_ROLE_FAILURE,
  DELETE_ROLE_SUCCESS,
  DELETE_ROLE_INPROGRESS,
  DELETE_ROLE,
  SAVE_USER,
  SAVE_USER_SUCCESS,
  SAVE_USER_INPROGRESS,
  SAVE_USER_FAILURE,
  DELETE_USER_FAILURE,
  DELETE_USER_INPROGRESS,
  DELETE_USER_SUCCESS,
  DELETE_USER,
  FETCH_USERS,
  FETCH_USERS_FAILED,
  FETCH_USERS_SUCCESS,
  FETCH_USERS_PROGRESS,
  FETCH_ROLES,
  FETCH_ROLES_FAILED,
  FETCH_ROLES_SUCCESS,
  FETCH_ROLES_PROGRESS,
  FETCH_STATUS_SUCCESS,
  FETCH_STATUS_FAILED,
  FETCH_STATUS,
  EXPAND_ACCOUNT_PANEL
};

const saveRole = role => ({ type: SAVE_ROLE, payload: { role } });
const removeRole = role => ({ type: DELETE_ROLE, payload: { role } });
const saveUser = user => ({ type: SAVE_USER, payload: { user } });
const deleteUser = user => ({ type: DELETE_USER, payload: { user } });
const fetchUsers = () => ({ type: FETCH_USERS });
const fetchRoles = () => ({ type: FETCH_ROLES });
const fetchStatuses = () => ({ type: FETCH_STATUS });
const expand = panel => ({
  type: EXPAND_ACCOUNT_PANEL,
  payload: { panel }
});

export default {
  removeRole,
  saveRole,
  saveUser,
  deleteUser,
  fetchRoles,
  fetchUsers,
  expand,
  fetchStatuses
};
